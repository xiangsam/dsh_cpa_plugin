...
/**
 * dsh releases this adapter is built and verified against. Keep in sync with
 * `peerDependencies`, the `dsh.compatibility` manifest field, and the README.
 * The 0.1.5-alpha line is a hard floor: the settings section uses
 * `settings.installSection`, which earlier dsh releases do not provide.
 */
const TESTED_DSH_VERSIONS = ["0.1.5-alpha.2"];
const SUPPORTED_DSH_RANGE = ">=0.1.5-alpha.2 <0.2.0-0";
...
/** Whether a dsh version sits inside the verified window (0.1.5 and later 0.1.x). */
function isSupportedDshVersion(version) {
	const [core = ""] = version.split("-", 1);
	const [major, minor, patch] = core.split(".").map(Number);
	return major === 0 && minor === 1 && patch >= 5;
}

/**
 * Warn once when the running dsh is outside the verified window. Never throws:
 * a mismatched profile must still boot so the message stays readable.
 */
function warnOnUnsupportedDsh(ctx) {
	...
	ctx.logger.error(
		`llm-cpa: unsupported dsh version ${version}; this release is verified against dsh `
		+ `${TESTED_DSH_VERSIONS.join(" and ")} (supported range ${SUPPORTED_DSH_RANGE}). `
		+ "The CPA provider may keep working, but the settings section and the Ctx slider need a supported dsh.",
	);
}
...
export function apply(ctx, config) {
	...
	ctx.inject(["settings"], (settingsCtx) => {
		...
		// Capability gate, not just a version string: earlier dsh releases expose
		// the settings service without `installSection`. Registering the adapter
		// with defaults keeps the provider usable instead of failing the tree.
		if (typeof settings?.installSection !== "function") {
			settingsCtx.logger.error(
				`llm-cpa: the running dsh has no settings.installSection (requires ${SUPPORTED_DSH_RANGE}); `
				+ "registering the CPA provider with the plugin config only — the Models page section and the live Ctx slider stay unavailable.",
			);
			return;
		}
